...
class sfMediaBrowserActions extends BasesfMediaBrowserActions
{
  ...
  public function executeFilescreatemulti(sfWebRequest $request) {
    $numero  = $request->getParameter('uploader_count');
    $dir = $request->getParameter('dir');

    for ($i=0 ; $i<$numero; $i++)
    {
        $file = $request->getParameter('uploader_'.$i.'_name');
        $nombre_tmp =$request->getParameter('uploader_'.$i.'_tmpname');
        $nombre_tmp_se = basename($request->getParameter('uploader_'.$i.'_tmpname'),'.tmp');
        $info = pathinfo($file);
  ...
  public function executePlupload(sfWebRequest $request){
      /**
     * upload.php
     *
	...
	// Settings
	$targetDir = realpath(sfConfig::get('sf_web_dir').'/sfMediaBrowserPlugin/plupload_tmp');
